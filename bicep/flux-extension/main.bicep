...
// module customRoleStuff './bicep/custom_stuff.bicep' = if (!empty(storageAccountName)) {
//   name: '${uniqueString(deployment().name, location)}-ManagedCluster-role'
//   params: {
...
module fluxConfiguration '../flux-configuration/main.bicep' = [
  for (fluxConfiguration, index) in (fluxConfigurations ?? []): {
    ...
    params: {
      ...
      sourceKind: contains(fluxConfiguration, 'gitRepository') ? 'GitRepository' : 'AzureBlob'
      name: fluxConfiguration.?name ?? toLower('${managedCluster.name}-fluxconfiguration${index}')
      bucket: fluxConfiguration.?bucket
      // azureBlob: !empty(fluxConfiguration.azureBlob) ? union(fluxConfiguration.azureBlob, {
      //   managedIdentity: {
      //     clientId: customRoleStuff.outputs.clientId
      //   }
      // }) : null
    ...
    dependsOn: !empty(storageAccountName) ? [
      extension
      // customRoleStuff
    ] : [
      extension
